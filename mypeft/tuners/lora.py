...
class LoraConfig(PeftConfig):
    ...
    expert_num: int = field(
        default=4,
        metadata={
            "help": "The number of experts in the expert gating mechanism. This is used to create the gating mechanism for the Lora layer."
        },
    )
    guide: bool = field(
        default=True,
        metadata={
            "help": "Whether to use the guide mechanism for the Lora layer. If set to True, the guide mechanism will be used."
        },
    )
    guide_mode: str = field(
        default="smmulsm",
        metadata={
            "help": "The guide mechanism to use for the Lora layer. Can be 'mulnorm' or 'addnorm'."
        },
    )
    ...
    **Attributes**:
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        kwargs = {
            ...
            "guide": lora_config.guide,
            "guide_mode": lora_config.guide_mode,
        }
        loaded_in_4bit = getattr(self.model, "is_loaded_in_4bit", False)
        loaded_in_8bit = getattr(self.model, "is_loaded_in_8bit", False)
    ...
    def _unload_and_optionally_merge(self, merge=True):
        ...
        # TODO: If merge, need to update this method to merge the lora layers
        key_list = [key for key, _ in self.model.named_modules()]
        for key in key_list:
            try:
                parent, target, target_name = _get_submodules(self.model, key)
...
class LoraLayer:
    def __init__(self, in_features: int, out_features: int, **kwargs):
        ...
        self.kwargs = kwargs
        self.expert_num = kwargs.get("expert_num", 4)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.guide = kwargs.pop("guide", True)
        self.guide_mode = kwargs.pop("guide_mode", "smmulsm")
        self.guide_temp = kwargs["guide_temp"] if "guide_temp" in kwargs else 5.0
        ...
        # -------------------------------------------------------- #
        # self.guide = kwargs.pop("guide", False)
        if self.guide:
            embed_loc = "./checkpoints/text_embeds.pt"
            text_embeds = torch.load(embed_loc)
            self.text_embeds = text_embeds.requires_grad_(False)
            self.lora_guidance_adapter = nn.ModuleDict({})
            self.lora_guidance_adapter.update(nn.ModuleDict({adapter_name: GuidanceAdapter(text_embeds.shape[1], self.expert_num)}))
        # -------------------------------------------------------- #
    ...
    def merge(self):
        ...
        if self.r[self.active_adapter] > 0:
            # TODO: merge moe layers
            # # expert_weight = self.lora_gate[self.active_adapter](x)
            # # same weight for all samples during merginf
            # expert_weight = torch.ones((1, self.expert_num)) * (1 / self.expert_num)
            # for i in range(self.expert_num):
            ...
            #     tmp_weight_data = transpose(lora_B_weights @ lora_A_weights, self.fan_in_fan_out,) * self.scaling[self.active_adapter]
            #     print("tmp_weight_data", tmp_weight_data.shape, "expert_weight", expert_weight.shape)
            #     self.weight.data += (
            #         transpose(
    ...
    def get_delta_weight(self, adapter):
        print("going to get delta weight", self.lora_B[adapter].weight.shape, self.lora_A[adapter].weight.shape, self.fan_in_fan_out, self.scaling[adapter])
        return (
            transpose(
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            # -------------------------------------------------------- #
            if self.guide:
                guidance_weight = self.lora_guidance_adapter[self.active_adapter](self.text_embeds.to(self.lora_A[self.active_adapter].loraA[0].mlp.weight.dtype).unsqueeze(1).to(x.device))
                
                raw_guidance_weight = F.softmax(guidance_weight / self.guide_temp, dim=-1)

                raw_expert_weight = F.softmax(expert_weight / self.guide_temp, dim=-1)
                expert_weight = raw_expert_weight * raw_guidance_weight
                expert_weight = expert_weight / expert_weight.sum(dim=-1, keepdim=True)

            # -------------------------------------------------------- #

            # print the gate name and expert weight
            for i in range(self.expert_num):
                tmp_res = self.lora_B[self.active_adapter].loraB[i](self.lora_A[self.active_adapter].loraA[i](self.lora_dropout[self.active_adapter](x))) * self.scaling[self.active_adapter]
                if self.guide:
                    if self.guide_mode == "smmulsm":
                        result += tmp_res * expert_weight[..., i].unsqueeze(-1)
                    else:
                        raise ValueError("Invalid guide mode")
                else:
                    result += tmp_res * expert_weight[..., i].unsqueeze(-1)
                # result += tmp_res * expert_weight[:, i].unsqueeze(-1)
        else:
...
class GuidanceAdapter(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        super().__init__()
        self.guidance_adapter = nn.Linear(input_size, output_size, bias=False)
    
    def forward(self, x):

        y = self.guidance_adapter(x)
